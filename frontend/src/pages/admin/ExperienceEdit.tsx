...
const ExperienceEdit = () => {
  ...
  const fetchExperience = () => {
    api.get('/experience').then(res => setExperience(res.data)).catch(console.error);
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const expData = {
      ...newExp,
      techStack: newExp.techStack.split(',').map(t => t.trim()).filter(t => t),
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto p-6 pt-20">
        ...
        <div className="mb-8 bg-white/10 backdrop-blur-md p-8 rounded-xl border border-white/20 shadow-xl">
          <h2 className="text-xl font-bold mb-6 text-white">Add New Experience</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-gray-300 text-sm font-semibold mb-2">Company Logo URL</label>
                <input
                  type="url"
                  placeholder="https://example.com/logo.png"
                  value={newExp.companyLogo}
                  onChange={(e) => setNewExp({ ...newExp, companyLogo: e.target.value })}
                  className="w-full p-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-colors"
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-gray-300 text-sm font-semibold mb-2">End Date (Leave empty if current)</label>
                <input
                  type="date"
            ...
            <div>
              <label className="block text-gray-300 text-sm font-semibold mb-2">Tech Stack (comma separated)</label>
              <input
                type="text"
            ...
            <div className="bg-white/5 p-4 rounded-lg border border-white/10">
              <label className="block text-gray-300 text-sm font-semibold mb-2">Key Achievements</label>
                ...
                <div className="space-y-2">
                  {achievementsList.map((achievement, index) => (
                    <div key={index} className="flex items-center gap-2 bg-white/5 p-2 rounded border border-white/10">
                      ...
                      <span className="flex-1 text-gray-300 text-sm">{achievement}</span>
                      <button
                        type="button"
            ...
            <button type="submit" className="bg-green-600 px-6 py-3 rounded-lg text-white font-bold hover:bg-green-700 transition-colors shadow-lg">
              Add Experience
            </button>
          </form>
        </div>
        ...
        <div className="space-y-4">
          {experience.map((exp) => (
            <div key={exp._id} className="bg-white/10 backdrop-blur-md p-6 rounded-xl border border-white/20 shadow-lg hover:bg-white/15 transition-all">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    {exp.companyLogo && (
                      <img src={exp.companyLogo} alt={exp.company} className="w-12 h-12 rounded-lg object-cover border border-white/20" />
                    )}
                    <div>
                  ...
                  {exp.achievements && exp.achievements.length > 0 && (
                    <div className="mt-3 bg-white/5 p-3 rounded border border-white/10">
                      <p className="text-xs text-purple-300 font-semibold mb-2">🏆 Achievements:</p>
                ...
                <button 
                  ...
                  className="bg-red-500/20 text-red-300 border border-red-500/50 px-3 py-1 rounded text-sm hover:bg-red-500/30 transition-colors ml-4"
                >
                  Delete
                </button>
              </div>

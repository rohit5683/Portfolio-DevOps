...
const UserManagement = () => {
  ...
  const handleResetToEmail = async () => {
    ...
    try {
      ...
      setUsers(
        users.map((u) =>
          u._id === resetMethodDialog.userId ? { ...u, mfaMethod: "email" } : u,
        ),
      );
      ...
      alert(
        error.response?.data?.message ||
          "Failed to reset MFA method. Please try again.",
      );
    }
  };
  ...
  const handleSetupTotp = async (userId: string, userEmail: string) => {
    ...
    try {
      ...
      alert(
        error.response?.data?.message ||
          "Failed to setup Google Authenticator. Please try again.",
      );
    } finally {
  ...
  const handleChangeMfaMethod = async (
    userId: string,
    method: "email" | "totp",
  ) => {
    try {
      ...
      // Update local state
      setUsers(
        users.map((u) => (u._id === userId ? { ...u, mfaMethod: method } : u)),
      );
      ...
      alert(
        error.response?.data?.message ||
          "Failed to update MFA method. Please try again.",
      );
    }
  };
    ...
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="relative z-10 container mx-auto px-3 py-6 md:px-4 md:py-8">
        ...
        <div className="flex justify-between items-center mb-6 md:mb-8">
          ...
          <div className="flex gap-2 md:gap-3">
            <button
              ...
              className="px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-base rounded-xl transition-colors flex items-center gap-1.5 md:gap-2"
            >
              <svg
            ...
            <button
              ...
              className="px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-base bg-white/5 hover:bg-white/10 text-white rounded-xl border border-white/10 transition-colors"
            >
        ...
        <div className="mb-6">
          <input
            ...
            className="w-full md:w-96 px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 shadow-lg"
          />
        </div>
                      ...
                      <td className="block md:table-cell px-2 md:px-6 py-2 md:py-4 md:text-center">
                        ...
                        {user.mfaEnabled ? (
                          <select
                            ...
                            onChange={(e) =>
                              handleChangeMfaMethod(
                                user._id,
                                e.target.value as "email" | "totp",
                              )
                            }
                          ...
                          >
                            ...
                            <option value="totp" className="bg-gray-800">
                              📱 TOTP
                            </option>
                          </select>
                        ) : (
                      ...
                      <td className="block md:table-cell px-2 md:px-6 py-4 md:py-4 md:text-center">
                        ...
                        <div className="flex flex-wrap items-center md:justify-center gap-2">
                          <button
                            ...
                            className="px-2 py-1 md:px-3 md:py-1 bg-green-500/20 hover:bg-green-500/30 text-green-300 rounded-xl transition-colors text-xs md:text-sm font-medium flex items-center gap-1 md:gap-2"
                          >
                          ...
                          <button
                            ...
                            className="px-2 py-1 md:px-3 md:py-1 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 rounded-xl transition-colors text-xs md:text-sm font-medium flex items-center gap-1 md:gap-2"
                          ...
                          <button
                            ...
                            className="px-2 py-1 md:px-3 md:py-1 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-xl transition-colors text-xs md:text-sm font-medium flex items-center gap-1 md:gap-2"
                          >
                            <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {confirmDialog?.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-white/20 rounded-3xl p-8 max-w-md mx-4 shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4">
              Confirm MFA Change
            </h3>
            <p className="text-gray-300 mb-6">
              Are you sure you want to{" "}
              {confirmDialog.currentStatus ? "disable" : "enable"} MFA for this
              user?
            </p>
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm md:text-base rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={confirmToggleMfa}
                className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm md:text-base rounded-xl transition-colors"
              >
                Confirm
              </button>
            </div>
      ...
      {deleteDialog.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-red-500/30 rounded-3xl p-8 max-w-md mx-4 shadow-2xl">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-red-600 hover:bg-red-700 text-white text-sm md:text-base rounded-xl transition-colors font-semibold"
              >
                Delete User
              </button>
            </div>
      ...
      {totpSetupDialog.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-green-500/30 rounded-3xl p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-gray-300 mb-4">
              ...
              <span className="text-green-400 font-semibold">
                {totpSetupDialog.email}
              </span>
            </p>

            {/* QR Code */}
            <div className="bg-white p-4 rounded-3xl mb-4 flex justify-center">
              <img
                src={totpSetupDialog.qrCode}
                alt="QR Code"
                className="w-48 h-48"
              />
            </div>
            ...
            <button
              onClick={() =>
                setTotpSetupDialog({ show: false, userId: "", email: "" })
              }
              className="w-full px-3 py-1.5 md:px-4 md:py-2 bg-green-600 hover:bg-green-700 text-white text-sm md:text-base rounded-xl transition-colors font-bold"
            >
              Done
            </button>
          </div>
      ...
      {showCreateDialog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/20 rounded-3xl p-8 max-w-md w-full">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 pr-12"
                    placeholder="••••••••"
                    required
                  />
                  <button
            ...
            <div className="flex gap-4 mt-6">
              <button
                ...
                disabled={creating}
                className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm md:text-base rounded-xl transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={creating}
                className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm md:text-base rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {creating ? (
                  <>
                    <svg
      ...
      {resetMethodDialog.show && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-3xl border border-white/10 p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-xl font-bold text-white mb-4">
              Reset MFA Method?
            </h3>
            <p className="text-gray-300 mb-6">
              Are you sure you want to reset the MFA method for{" "}
              <strong>{resetMethodDialog.email}</strong> to Email? This will
              disable Google Authenticator for this user.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm md:text-base rounded-xl transition-colors font-bold"
              >
                Reset to Email
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-3xl border border-white/10 p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-gray-300 mb-4">
              ...
              <strong>{changePasswordDialog.email}</strong>
            </p>
            <input
              type="password"
              value={newPassword}
              onChange={(e) => setNewPassword(e.target.value)}
              className="w-full px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 mb-6"
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-3 py-1.5 md:px-4 md:py-2 bg-yellow-600 hover:bg-yellow-700 text-white text-sm md:text-base rounded-xl transition-colors font-bold"
              >
                Update Password
              </button>
            </div>
